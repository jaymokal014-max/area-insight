...
// Fallback data in case the live API server is offline or limits are reached
const fallbackData = [
...
const dict = {
  en: {
    ...
    fetchingData: "Authenticating API Key & Fetching Data...",
  ...
  mr: {
    ...
    autoTheft: "वाहन चोरी",
    liveAPI: "थेट डेटा (data.gov.in)",
    fetchingData: "डेटा प्राप्त करत आहोत...",
...
export default function App() {
  ...
  // ==============================================================
  // LIVE API INTEGRATION LOGIC (Data.gov.in)
  // Fully Functional & Mapped Live Data Parsing
  // ==============================================================
  useEffect(() => {
    const fetchGovernmentData = async () => {
      try {
        ...
        if (jsonData && jsonData.records && jsonData.records.length > 0) {
          ...
          // DATA MAPPER: Translate messy government columns into our App's format
          const liveData = jsonData.records.map(record => {
            return {
              // Map State Name (fallback to "Unknown" if missing)
              state: record.state_ut || record.state_name || record['state/ut'] || record.state || "Unknown",
              
              // Map Crime Cases over 3 years
              y2020: parseFloat(record.cases_2020 || record['2020'] || record.y2020 || record.total_cognizable_ipc_crimes_2020 || 0),
              y2021: parseFloat(record.cases_2021 || record['2021'] || record.y2021 || record.total_cognizable_ipc_crimes_2021 || 0),
              y2022: parseFloat(record.cases_2022 || record['2022'] || record.y2022 || record.total_cognizable_ipc_crimes_2022 || 0),
              
              // Map Population, Rate, and Charge Sheeting
              pop: parseFloat(record.population_in_lakhs_2022 || record.projected_mid_year_population_in_lakhs_2022 || record.population || 0),
              rate: parseFloat(record.rate_of_cognizable_crimes_ipc_2022 || record.crime_rate_2022 || record.rate || 0),
              charge: parseFloat(record.charge_sheeting_rate_2022 || record.charge_rate || record.charge || 0)
            };
          });

          // Verify mapping worked successfully
          if (liveData[0].state !== "Unknown") {
             console.log("Live API Data Successfully Mapped!");
             setApiStatus("success");
             setData(liveData); // ACTUALLY USING LIVE DATA IN THE UI NOW
          } else {
             throw new Error("Live data schema has changed unexpectedly");
          }
        } else {
          throw new Error("Invalid API structure returned");
        }
      } catch (error) {
        console.warn("API Fetch Failed (CORS or schema change). Applying Offline Fallback Data.", error);
  ...
  const stateData = useMemo(() => data.find(d => d.state === selectedState) || data[0] || fallbackData[13], [data, selectedState]); 
  ...
  const startListening = () => {
    ...
    recognition.onresult = (event) => {
      ...
      // Ensure we don't accidentally select the national average
      const found = data.find(d => 
        !d.state.includes("Total") && !d.state.includes("India") && transcript.includes(d.state.toLowerCase())
      );
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 font-sans pb-32 relative selection:bg-blue-500/30 selection:text-blue-200">
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest" aria-live="polite">
                  {apiStatus === 'fallback' ? t.offlineReady : t.liveAPI}
                </p>
              </div>
            </div>
          </div>
      ...
      <main className="max-w-5xl mx-auto p-4 mt-4 grid grid-cols-1 lg:grid-cols-3 gap-6" key={animateKey}>
          ...
          <div className="bg-slate-800 p-6 rounded-[2rem] shadow-xl border border-white/5 ring-1 ring-white/10">
            ...
            <div className="border-t border-white/5 pt-6">
              ...
              <div className="relative">
                 ...
                 >
                   {/* Filter out aggregate rows from the dropdown */}
                   {data.filter(d => !d.state.includes("Total") && !d.state.includes("India")).map((d) => (
        ...
        <section className="lg:col-span-2 space-y-6 animate-[fadeIn_0.5s_ease-out]" aria-label="Safety Metrics and Charts">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div className="bg-slate-800 p-6 rounded-[2rem] shadow-lg border border-white/5 ring-1 ring-white/10">
              ...
              <div className="space-y-4">
                ...
                {/* Visual rendering, hidden from screen readers. Screen readers read the span below. */}
                ...
                {/* Screen Reader Only Text for Breakdowns */}
